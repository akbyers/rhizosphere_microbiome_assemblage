...
ASV.ITS <- read.csv("~/Documents/Alexa_TRM_analysis/ITS sequencing/ASV_counts_cleaned.csv")
Taxa.ITS <- read.csv("~/Documents/Alexa_TRM_analysis/ITS sequencing/Taxonomy_cleaned.csv")
Sample.ITS <- read.csv("~/Documents/Alexa_TRM_analysis/ITS sequencing/ITS_samples_post_filtering.csv")
ITS.tree <- read.tree("~/Documents/Alexa_TRM_analysis/Phylotree construction/ITS_fasttreeRooted/tree.nwk")
...
ASV.16S <- read.csv("~/Documents/Alexa_TRM_analysis/16S data/DADA2 outputs/DADA2 outputs/ASV_counts_cleaned.csv")
Taxa.16S <- read.csv("~/Documents/Alexa_TRM_analysis/16S data/DADA2 outputs/DADA2 outputs/Taxonomy_cleaned.csv")
Sample.16S <- read.csv("~/Documents/Alexa_TRM_analysis/16S data/DADA2 outputs/16S_samples_post_filtering.csv")
x16S.tree <- read.tree("~/Documents/Alexa_TRM_analysis/Phylotree construction/16s_fasttreeRooted/tree.nwk")
ASV.16S = ASV.16S[,-1]
...
#add phy_tree back in phyloseq object (had to remove for PIME analysis)
```{r ITS}
prevalence.80_ITS <- merge_phyloseq(prevalence.80_ITS, phy_tree(ITS.tree))
#rename phyloseq object
physeq.rarefied_ITS <- prevalence.80_ITS
```
...
write.csv(Adonis.ITS.Results, "~/Documents/Alexa_TRM_analysis/January 2022/Adonis_ITS.csv")
...
png("~/Documents/Alexa_TRM_analysis/January 2022/ITS_NMDS.png", width = 800, height = 600)
its_plot
```
...
write.csv(Mantel.ITS.Results, "~/Documents/Alexa_TRM_analysis/January 2022/Mantel_ITS.csv")
```
...
write.csv(betadisp_unifrac_distances_ITS, "~/Documents/Alexa_TRM_analysis/January 2022/betadispDistances_ITS.csv")
...
write.csv(permu_betadisp_its.Results, "~/Documents/Alexa_TRM_analysis/January 2022/Perm_Betadisp_its.csv")
...
capture.output(Permu_betadisp_its.pairwise, file = "~/Documents/Alexa_TRM_analysis/January 2022/Permu_Pairwise_Betadisp_its.csv")
```
...
#add phy_tree back in phyloseq object (had to remove for PIME analysis)
```{r}
prevalence.85_16S <- merge_phyloseq(prevalence.85_16S, phy_tree(x16S.tree))
#rename phyloseq object
physeq.rarefied.16S <- prevalence.85_16S
```
...
write.csv(Adonis.16S.Results, "~/Documents/Alexa_TRM_analysis/January 2022/Adonis_16S.csv")
...
capture.output(Pairwise_adonis_16S2, "~/Documents/Alexa_TRM_analysis/January 2022/pairwise_Adonis_16S.csv")
```
##16S nmds plots
```{r}
x16S_plot = plot_ordination(physeq.rarefied.16S, ordination.16S, type="samples", color="PlantID") +   nmds_layout + 
    geom_point(size=4, alpha= 1)+
    geom_convexhull(aes(fill= PlantID), alpha=.6)
png("~/Documents/Alexa_TRM_analysis/January 2022/16S_NMDS.png", width = 800, height = 600)
x16S_plot
```
...
write.csv(Mantel.16S.Results, "~/Documents/Alexa_TRM_analysis/January 2022/Mantel_16S.csv")
```
...
write.csv(betadisp_distances_16S, "~/Documents/Alexa_TRM_analysis/January 2022/betadispDistances_16S.csv")
...
write.csv(permu_betadisp_16s.Results, "~/Documents/Alexa_TRM_analysis/January 2022/Perm_Betadisp_16s.csv")
...
capture.output(Permu_betadisp_16s.pairwise, file = "~/Documents/Alexa_TRM_analysis/January 2022/Permu_Pairwise_Betadisp_16s.csv")
```
##betadispersion summary statistics
```{r}
...
matK.dist <- read.csv("~/Documents/Alexa_TRM_analysis/matK_dist.csv")
...
tiff("~/Documents/Alexa_TRM_analysis/January 2022/16S_Unifrac_matK.tiff", units="in", width=10, height=8, res=300)
...
tiff("~/Documents/Alexa_TRM_analysis/January 2022/ITS_Unifrac_matK.tiff", units="in", width=10, height=8, res=300)
...
tiff("~/Documents/Alexa_TRM_analysis/January 2022/ITS_VarPart.tiff", units="in", width=8, height=5, res=300)
...
rda.16S.result2 <- capscale(x16S.community ~ as.matrix(soil.env.16S) + Condition(mm1_16S_plantID) + Condition(mm2_16S_PLHT) + Condition(mm3.16S))
...
tiff("~/Documents/Alexa_TRM_analysis/January 2022/16S_VarPart.tiff", units="in", width=8, height=5, res=300)
